...
# Method for wv4 (wikivitals 4 - Telecom), wos (Web of Science), wv5-04-2022 (wikivitals 5 - perso)
def load_JSONs(
        data_args):
    """
    ...
    Output:
    ds (DatasetDict): dataset dictionnary with 3 splits named 'train', 'validation', and 'test' 
    and 2 columns 'texts' and 'labels'
    """
    data_files = {
    ...
    # Retrieve text and create 'texts' column
    ...
    column_names = column_names['train']
    ds = ds.remove_columns([i for i in column_names if not i in ["texts", "labels"]])
    return ds


def load_wv5_04_2022(data_args):
    data_path = f"{data_args.path_base}{data_args.train_file}"
...
def prepare_dataset(
    ...
    # At this point, we have a dataset with 2 columns, the text field and the label field
    # Labels are stored as follow : ['A', 'child_of_A', 'grandchild_of_A', ...]
...
#     ds = ds.remove_columns(['topic', 'keyword'])

#     # At this point, we have a dataset with 2 columns, the text field and the label field
#     # Labels are stored as follow : ['A', 'child_of_A', 'grandchild_of_A', ...]
#     # The text must be the first element in a list of str
...
#     ds = ds.remove_columns([inputs_col, labels_col])
